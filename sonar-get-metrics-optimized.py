...
df["sonar_component_key"] = df["file_name"].map(sonar_files)
...
for index, row in df_filtered.iterrows():
    ...
    full_code = row["full_code"]  # 🔹 Read from the dataset instead of reading a file
    snippet = "\n".join(full_code.split("\n")[:5]) + "..."  # 🔹 Generate snippet

    # ✅ Fetch SonarCloud metrics for this component
    metrics = get_all_sonar_metrics(component_key)

    # ✅ Store full results
    full_row = {
        "file_name": file_name,
        "full_code": full_code  # 🔹 No need to read from `file_path`
    }
    full_row.update(metrics)
    full_results.append(full_row)

    # ✅ Store snippet results
    snippet_row = {
        "file_name": file_name,
        "snippet": snippet
    }
...
df_full.to_csv("training_optimized_dataset_metrics.csv", index=False)
df_snip.to_csv("training_optimized_dataset_metrics_snippetcode.csv", index=False)
